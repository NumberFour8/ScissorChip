...
void compress(monpoint* R0,monpoint* R1)
{
      ...
      FROM_MONREP(R0->xed,R0->z);    // Convert X from Montgomery representation
      FROM_MONREP(R0->yed,R1->x);    // Convert Y from Montgomery representation
      
    #elif defined(EXTENDED)
      
      bigint t1;
      
      mp_invert(R0->t,R0->z);        // Z^-1
      mp_mulmod(t1,R0->y,R0->t);     // Y*Z^-1
      mp_mulmod(R0->y,R0->x,R0->t);  // X*Z^-1
      
      FROM_MONREP(R0->xed,R0->y);
      FROM_MONREP(R0->yed,t1);
    
    #elif defined(PROJECTIVE)
    
      bigint t1,t2;
      
      mp_invert(t1,R0->z);           // Z^-1
      mp_mulmod(R0->z,R0->x,t1);     // X*Z^-1
      mp_mulmod(t2,R0->y,t1);        // Y*Z^-1
      
      FROM_MONREP(R0->xed,R0->z);
      FROM_MONREP(R0->yed,t2);
    
    #elif defined(INVERTED)
      
      bigint t1,t2,t3;
      
      mp_mulmod(t1,R0->x,R0->y);
      mp_invert(t2,t1);
      
      mp_mulmod(t1,t2,R0->x);
      mp_mulmod(t3,t1,R0->z); // Y
      
      mp_mulmod(t1,t2,R0->z);
      mp_mulmod(t2,t1,R0->y); // X
      
      FROM_MONREP(R0->xed,t2);
      FROM_MONREP(R0->yed,t3);
      
    #endif  
                 
    R0->yed[15] &= 0x7FFF;               // Clear most-significant bit
    R0->yed[15] |= (R0->xed[0]&1) << 15; // Add the parity bit
}